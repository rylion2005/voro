...
import com.voro.speech.mouth.TtsConfigurations;
 ...
 * <p>
 * 示例使用代码：
 * IntegrationCheck.getInstance(getApplicationContext()).check(initConfig, new Handler() {
 *
 * @Override public void handleMessage(Message msg) {
 * if (msg.what == 100) {
 * IntegrationCheck autoCheck = (IntegrationCheck) msg.obj;
 * synchronized (autoCheck) {
 * String message = autoCheck.obtainDebugMessage();
 * toPrint(message); // 可以用下面一行替代，在logcat中查看代码
 * //Log.w("IntegrationCheckMessage",message);
 * }
 * }
 * }
 * <p>
...
public class IntegrationCheck {
    ...
    private static IntegrationCheck gInstance;
     ...
     * @return
     */
    public static IntegrationCheck getInstance(Context context) {
        if (gInstance == null || IntegrationCheck.msContext != context) {
            gInstance = new IntegrationCheck(context);
        }
        return gInstance;
    }
    ...
    public void check(final Handler handler) {
        Thread t = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                AuthKeys auth = AuthKeys.getInstance(msContext);
                IntegrationCheck obj = innerCheck(configs, auth);
                isFinished = true;
    ...
    private IntegrationCheck innerCheck(TtsConfigurations config, AuthKeys auth) {
        checks.put("检查申请的Android权限", new PermissionCheck(msContext));
        checks.put("检查4个so文件是否存在", new JniCheck(msContext));
        checks.put("检查AppId AppKey SecretKey",
                new AppInfoCheck(auth.getAppId(), auth.getApiKey(), auth.getSecretKey()));
        checks.put("检查包名", new ApplicationIdCheck(msContext, auth.getAppId()));
    ...
    private IntegrationCheck(Context context) {
        msContext = context;
